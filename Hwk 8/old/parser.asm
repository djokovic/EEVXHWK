...
ParseCheckError:
    CMP     Errorflag, FALSE    ; Was there an error from the FSM action?
...
ParseRecordError:
    ...
    MOV     AH, FMS_ERROR       ; Indicate that this was an FSM error (This
    ...
    JMP     ParseNeedReset      ; We just got error, thus immediately exit this cmd path
                                ; and go back to ST_INTIAL to wait for VALID next cmd.
...
Concat_Num_Test:   
    ...
    CMP     AX, MAX_MAG             ; Does the mag fit the # of bits restriction?
    JG      Concat_MagTooBig        ; No, error
    ;JLE    Concat_success          ; 
...
SetRelSpeedPos:
    ADD     AX, magnitude           ;
    JNC     SetRelSpeedWrite        ; Speed is valid
    ;JC      SetRelWentOver         ; Speed went 'too big' and not valid
...
RotAbsTurrAng       PROC    NEAR
    ...
    CALL    SetRelTurretAngle       ;  Pass angle in AX [0 to 360]
...
LaserControl    PROC    NEAR
    ...
    CBW                     ; AL already has TRUE or FALSE, so convert to AX
...
StateTable	LABEL	TRANSITION_ENTRY
	...
	%TRANSITION(ST_RTA_INIT, no_op)	    ;TOKEN_T - Rot Turr Angl
...
;-----------------------------Setting Absolute Speed----------------------------------	
	...
	%TRANSITION(ST_SAS, no_op)		    ;TOKEN_POS - '+' Accepted, but effectively ignored
	%TRANSITION(ST_INITIAL, SetError)		;TOKEN_NEG - '-'
...
;-----------------------------Setting Relative Speed----------------------------------	
	...
	%TRANSITION(ST_SRS, SetSign)        ;TOKEN_POS - '+' Wait for sign
	%TRANSITION(ST_SRS, SetSign)        ;TOKEN_NEG - '-' Wait for sign
...
;-----------------------------Setting Direction Speed----------------------------------	
	...
	%TRANSITION(ST_DIR, SetSign)   ;TOKEN_POS - '+' Wait for sign
	%TRANSITION(ST_DIR, SetSign)   ;TOKEN_NEG - '-' Wait for sign
...
;-----------------------------Rotating Turrent Angle----------------------------------	
	...
	%TRANSITION(ST_RTA_REL, SetSign)    ;TOKEN_POS - '+' Wait for sign
	%TRANSITION(ST_RTA_REL, SetSign)    ;TOKEN_NEG - '-' Wait for sign
...
;-----------------------------Elevation of Turret----------------------------------	
	...
	%TRANSITION(ST_STEA, SetSign)   ;TOKEN_POS - '+' Wait for sign
	%TRANSITION(ST_STEA, SetSign)   ;TOKEN_NEG - '-' Wait for sign
...
Error_flag      DW      ?               ; Holds error type
magnitude       DW      ?               ; Shared magnitude (can be angle, speed), unsigned 15-bit val
